...
def inkycal_config():
    ...
    if form.validate_on_submit():
        ...
        info_height = (
            int(request.form.get("info_section_height"))
        ...
        display_size = Display.get_display_size(
            model
        )  # returns width,height but flipping these for vertical mode
        ...
        module_sizes = [
            int(request.form.get(f"module{i}_height"))
                            ...
                            for key in modules["optional"]:
                                if request.form.get(f"module{i}_{key}"):
                                    ...
                                    if "default" in modules["optional"][key]:
                                        conf["config"][key] = modules["optional"][key][
                                            "default"
                                        ]
                                    else:
        ...
        try:
            ...
            response = Response(
                user_settings, mimetype="application/json", direct_passthrough=True
            )
            response.headers[
                "Content-Disposition"
    ...
    return render_template(
        "inkycal-config-v2-0-0.html", title="Inkycal-Setup", conf=settings, form=form
    )
