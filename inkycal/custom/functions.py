...
top_level = os.path.dirname(os.path.abspath(os.path.dirname(__file__))).split(
    os.sep + "inkycal"
)[0]
...
def write(image, xy, box_size, text, font=None, **kwargs):
    ...
    if autofit or (fill_width != 1.0) or (fill_height != 0.8):
        ...
        while text_width < int(box_width * fill_width) and text_height < int(
            box_height * fill_height
        ):
            size += 1
            font = ImageFont.truetype(font.path, size)
            text_width, text_height = font.getsize(text)[0], font.getsize("hg")[1]
...
def draw_border(image, xy, size, radius=5, thickness=1, shrinkage=(0.1, 0.1)):
    ...
    if radius != 0:
        ...
        c5, c6 = ((x + width) - diameter, (y + height) - diameter), (
            x + width,
            y + height,
        )
