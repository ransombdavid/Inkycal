...
class Inkycal:
    ...
    def __init__(self, settings_path=None, render=True, optimize=True):
        """Initialise Inkycal"""
        ...
        if settings_path:
            try:
                ...
                print(
                    "No settings file found in given path\n"
        ...
        for module in settings["modules"]:
            ...
            # If a module was not found, print an error message
                ...
                print(
                    f'Could not find module: "{module}". Please try to import manually'
                )
    ...
    def countdown(self, interval_mins=None):
        ...
        now = arrow.now()
        update_timings = [
            (60 - int(interval_mins) * updates)
            for updates in range(60 // int(interval_mins))
        ][::-1]
    ...
    def test(self):
        ...
        for number, module in enumerate(self.modules, start=1):
            name = module.name
    ...
    def run(self):
        ...
        while True:
            ...
            print(
                f"Date: {current_time.format('D MMM YY')} | "
    ...
    def run_once(self):
        """Runs main program to build images and display them once.

        Inkycal generates the image from all modules, assembles them in one image,
        and refresh the E-Paper.
        """
    ...
    def _assemble(self):
        ...
        for number in range(1, self._module_number):
            ...
            if os.path.exists(im1_path):
                ...
                section_size = [
                    i for i in self.settings["modules"] if i["position"] == number
                ][0]["config"]["size"]
        ...
        if self.settings["info_section"] == True:
            ...
            info_width = width
            font = self.font = ImageFont.truetype(fonts["NotoSansUI-Regular"], size=14)

            info_x = im_black.size[1] - info_height
            write(
                im_black, (0, info_x), (info_width, info_height), self.info, font=font
            )
    ...
    def add_module(cls, filepath):
        ...
        if not module_folder in filepath:
            raise Exception(
                f"Your module should be in {module_folder} "
        ...
        if not classname:
            raise TypeError(
                "your module doesn't seem to be a correct inkycal module.."
        ...
        print(
            f"Your module '{filename}' with class '{classname}' has been added "
            "successfully! Hooray!"
        )
    ...
    def remove_module(cls, filename, remove_file=True):
        ...
        try:
            with open(f"{module_folder}/{filename}", mode="r") as file:
                ...
                if not classname:
                    print(
                        "The module you are trying to remove is not an inkycal module.. "
                        "Not removing it."
                    )
                    return
